...
<form method="POST" action="{{ $case->exists ? route('cases.update',$case) : route('cases.store') }}" class="row g-3">
    ...
    <div class="col-md-4"><label class="form-label">Category</label><select class="form-select" name="category" required>@foreach($categories as $category)<option @selected(old('category',$case->category ?? 'Civil')===$category) value="{{ $category }}">{{ $category }}</option>@endforeach</select></div>
    ...
    <div class="col-md-3"><label class="form-label">Priority</label><select class="form-select" name="priority">@foreach($priorities as $p)<option @selected(old('priority',$case->priority ?? 'normal')===$p) value="{{ $p }}">{{ ucfirst($p) }}</option>@endforeach</select></div>
    <div class="col-md-4"><label class="form-label">Status</label><select class="form-select" name="status">@foreach($statuses as $s)<option @selected(old('status',$case->status ?? 'filed')===$s) value="{{ $s }}">{{ str_replace('_',' ',ucfirst($s)) }}</option>@endforeach</select></div>
