...
<form method="POST" action="{{ $case->exists ? route('cases.update',$case) : route('cases.store') }}" class="row g-3">
    ...
    <div class="col-md-4"><label class="form-label">Status</label><select class="form-select" name="status">@foreach(['filed','accepted','under_review','hearing_scheduled','in_progress','judgment_reserved','disposed','dismissed'] as $s)<option @selected(old('status',$case->status ?? 'filed')===$s) value="{{ $s }}">{{ str_replace('_',' ',ucfirst($s)) }}</option>@endforeach</select></div>
